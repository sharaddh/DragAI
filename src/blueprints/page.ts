...
function getColorClasses(color: string): Record<string, string> {
  const colors: Record<string, Record<string, string>> = {
    blue: { accent: "blue", accentHex: "#3B82F6", gradientFrom: "from-blue-600", gradientTo: "to-indigo-700", gradientText: "from-blue-400 to-indigo-300", ring: "ring-blue-500", bgLight: "bg-blue-50", bgDark: "bg-blue-900/20", textAccent: "text-blue-400", borderAccent: "border-blue-500/30", glow: "rgba(59,130,246,0.3)" },
    indigo: { accent: "indigo", accentHex: "#6366F1", gradientFrom: "from-indigo-600", gradientTo: "to-purple-700", gradientText: "from-indigo-400 to-purple-300", ring: "ring-indigo-500", bgLight: "bg-indigo-50", bgDark: "bg-indigo-900/20", textAccent: "text-indigo-400", borderAccent: "border-indigo-500/30", glow: "rgba(99,102,241,0.3)" },
    purple: { accent: "purple", accentHex: "#8B5CF6", gradientFrom: "from-purple-600", gradientTo: "to-pink-700", gradientText: "from-purple-400 to-pink-300", ring: "ring-purple-500", bgLight: "bg-purple-50", bgDark: "bg-purple-900/20", textAccent: "text-purple-400", borderAccent: "border-purple-500/30", glow: "rgba(139,92,246,0.3)" },
    pink: { accent: "pink", accentHex: "#EC4899", gradientFrom: "from-pink-600", gradientTo: "to-rose-700", gradientText: "from-pink-400 to-rose-300", ring: "ring-pink-500", bgLight: "bg-pink-50", bgDark: "bg-pink-900/20", textAccent: "text-pink-400", borderAccent: "border-pink-500/30", glow: "rgba(236,72,153,0.3)" },
    teal: { accent: "teal", accentHex: "#14B8A6", gradientFrom: "from-teal-600", gradientTo: "to-cyan-700", gradientText: "from-teal-400 to-cyan-300", ring: "ring-teal-500", bgLight: "bg-teal-50", bgDark: "bg-teal-900/20", textAccent: "text-teal-400", borderAccent: "border-teal-500/30", glow: "rgba(20,184,166,0.3)" },
    green: { accent: "green", accentHex: "#22C55E", gradientFrom: "from-green-600", gradientTo: "to-emerald-700", gradientText: "from-green-400 to-emerald-300", ring: "ring-green-500", bgLight: "bg-green-50", bgDark: "bg-green-900/20", textAccent: "text-green-400", borderAccent: "border-green-500/30", glow: "rgba(34,197,94,0.3)" },
    orange: { accent: "orange", accentHex: "#F97316", gradientFrom: "from-orange-600", gradientTo: "to-amber-700", gradientText: "from-orange-400 to-amber-300", ring: "ring-orange-500", bgLight: "bg-orange-50", bgDark: "bg-orange-900/20", textAccent: "text-orange-400", borderAccent: "border-orange-500/30", glow: "rgba(249,115,22,0.3)" },
    red: { accent: "red", accentHex: "#EF4444", gradientFrom: "from-red-600", gradientTo: "to-rose-700", gradientText: "from-red-400 to-rose-300", ring: "ring-red-500", bgLight: "bg-red-50", bgDark: "bg-red-900/20", textAccent: "text-red-400", borderAccent: "border-red-500/30", glow: "rgba(239,68,68,0.3)" },
  };
  return colors[color] || colors.indigo;
}

const features = [
  { title: "Lightning Fast", desc: "Optimized for performance with sub-second response times and instant updates across all devices.", icon: "⚡" },
  { title: "Secure by Default", desc: "Enterprise-grade security with end-to-end encryption, SOC 2 compliance, and zero-trust architecture.", icon: "🛡️" },
  { title: "Smart Analytics", desc: "Real-time insights with AI-powered dashboards, custom reports, and predictive trend analysis.", icon: "📊" },
  { title: "Global Scale", desc: "99.99% uptime with auto-scaling infrastructure across 30+ global data centers.", icon: "🌍" },
  { title: "Team Collaboration", desc: "Built for teams with real-time editing, version control, and granular permission management.", icon: "🤝" },
  { title: "API First", desc: "RESTful and GraphQL APIs with webhook support, SDKs in 12 languages, and detailed documentation.", icon: "🔌" },
];
...
const testimonials = [
  { quote: "This platform transformed our workflow. The automation features alone saved us over 40 hours per week.", name: "Sarah Chen", role: "CTO, TechVentures", avatar: "SC" },
  { quote: "We evaluated 12 solutions before choosing this one. The scalability and support are unmatched in the industry.", name: "Marcus Rivera", role: "Engineering Lead, DataFlow", avatar: "MR" },
  { quote: "The analytics dashboard gives us insights we never had before. It's like having a data science team built-in.", name: "Emily Nakamura", role: "VP Product, CloudScale", avatar: "EN" },
];
...
const footerLinks = [
  ...
  { heading: "Resources", links: ["Documentation", "API Reference", "Guides", "Community", "Status"] },
  { heading: "Legal", links: ["Privacy", "Terms", "Security", "Cookies", "GDPR"] },
];

export function generatePage(config: ComponentConfig): ComponentResult {
  const name = config.name || "LandingPage";
...
export default function ${name}({
  ...
  dark: initialDark = ${darkMode ? "true" : "false"},
  onThemeToggle,
}: ${name}Props) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('${name.toLowerCase()}-theme');
    if (stored !== null) setDark(stored === 'dark');
  }, []);

  useEffect(() => {
    localStorage.setItem('${name.toLowerCase()}-theme', dark ? 'dark' : 'light');
  ...
  return (
    <div className={dark ? 'dark' : ''}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 md:h-20">
            <a href="#" className="flex items-center gap-2 group">
              <div className={\`w-8 h-8 rounded-lg bg-gradient-to-br ${c.gradientFrom} ${c.gradientTo} flex items-center justify-center text-white font-bold text-sm transition-transform group-hover:scale-110\`}>A</div>
                ...
                <a key={link.id} href={\`#\${link.id}\`}
                  ...
                  className={\`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 \${
                    activeSection === link.id
                      ? \`${c.textAccent} bg-${c.accent}-50 dark:bg-${c.accent}-900/20\`
        ...
        <div className={\`md:hidden transition-all duration-300 overflow-hidden \${
          ...
          <div className={\`px-4 py-3 space-y-1 border-t \${
            ...
            <a href="#cta" onClick={(e) => { e.preventDefault(); document.getElementById('cta')?.scrollIntoView({ behavior: 'smooth' }); setMobileOpen(false); }}
              className={\`block px-4 py-2.5 rounded-lg text-sm font-semibold text-center text-white bg-gradient-to-r ${c.gradientFrom} ${c.gradientTo}\`}
            >Get Started</a>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className={\`inline-block px-4 py-1.5 rounded-full text-xs font-semibold mb-4 \${
              dark ? 'bg-${c.accent}-900/30 text-${c.accent}-300' : 'bg-${c.accent}-100 text-${c.accent}-700'
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ${features.map((f, i) => `            <div className={\`group relative p-6 rounded-2xl border transition-all duration-500 hover:scale-[1.02] hover:shadow-xl \${
              visibleSections.has('features') ? 'animate-fade-in-up' : 'opacity-0'
            } stagger-${i + 1} \${
              dark
                ? 'border-white/5 bg-gray-800/50 hover:border-white/20 hover:shadow-${c.accent}-500/10'
                : 'border-gray-100 bg-white hover:border-gray-200 hover:shadow-${c.accent}-500/10'
            }\`}
            ...
            >
              <div className={\`w-12 h-12 rounded-xl flex items-center justify-center text-xl mb-4 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 \${
                dark ? 'bg-${c.accent}-900/30' : 'bg-${c.accent}-100'
              }\`}>{'${f.icon}'}</div>
              ...
              <div className={\`mt-4 flex items-center gap-1 text-sm font-medium \${
                dark ? 'text-${c.accent}-400' : 'text-${c.accent}-600'
      ...
      <section id="stats" className={\`relative py-20 overflow-hidden \${
        dark ? 'bg-gray-950' : 'bg-${c.accent}-50'
      }\`} data-observe>
        <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            ${stats.map((s, i) => `            <div className={\`text-center \${countsVisible ? 'animate-count-up' : 'opacity-0'}\`} style={{ animationDelay: '${(i * 0.15).toFixed(2)}s' }}>
              <div className={\`text-3xl md:text-5xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r ${c.gradientFrom} ${c.gradientTo}\`}>${s.value}</div>
              <div className={\`text-sm \${
                dark ? 'text-gray-400' : 'text-gray-600'
              }\`}>${s.label}</div>
            </div>`).join('\n')}
          ...
          <div className="relative">
            <div className={\`relative p-8 md:p-12 rounded-2xl border \${
              ...
              <div className="flex items-center justify-center gap-3">
                <div className={\`w-10 h-10 rounded-full bg-gradient-to-br ${c.gradientFrom} ${c.gradientTo} flex items-center justify-center text-white text-sm font-bold\`}>{testimonials[activeTestimonial].avatar}</div>
            ...
            <div className="flex justify-center gap-2 mt-8">
              ${testimonials.map((_, i) => `              <button key={i} onClick={() => setActiveTestimonial(i)}
                className={\`w-2.5 h-2.5 rounded-full transition-all duration-300 \${
                  activeTestimonial === i
                    ? \`w-8 bg-${c.accent}-500\`
                    : \`\${dark ? 'bg-gray-700' : 'bg-gray-300'} hover:bg-${c.accent}-300\`
                }\`}
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input type="email" placeholder="Enter your email"
                className={\`flex-1 px-5 py-3 rounded-xl text-sm border outline-none transition-all duration-200 focus:ring-2 \${
                  dark
                    ? 'bg-gray-900 border-white/10 text-white placeholder-gray-500 focus:border-${c.accent}-500 focus:ring-${c.accent}-500/20'
                    : 'bg-white border-gray-200 text-gray-900 placeholder-gray-400 focus:border-${c.accent}-500 focus:ring-${c.accent}-500/20'
                }\`}
              />
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            <div className="col-span-2 md:col-span-1">
              <a href="#" className="flex items-center gap-2 mb-4">
                <div className={\`w-8 h-8 rounded-lg bg-gradient-to-br ${c.gradientFrom} ${c.gradientTo} flex items-center justify-center text-white font-bold text-sm\`}>A</div>
              ...
              <div className="flex gap-3">
                {['twitter', 'github', 'linkedin'].map(platform => (
                  <a key={platform} href="#" className={\`p-2 rounded-lg transition-all hover:scale-110 \${
                    dark ? 'text-gray-500 hover:text-${c.accent}-400 hover:bg-${c.accent}-900/20' : 'text-gray-400 hover:text-${c.accent}-600 hover:bg-${c.accent}-100'
                  }\`} aria-label={platform}>
